...
all_articles2 <- all_articles[10:12]

all_articles_uncleaned2 <- all_articles %>% discard(is.null) %>%  
  map(unlist) %>% map(~setNames(.x, make.unique(names(.x)))) %>% 
  map_dfr(as_tibble_row)

all_articles_uncleaned2 <- all_articles %>%
  discard(is.null) %>%
  map_df(~ {
    x <- .x
    tibble(
      web_url = x$web_url %||% NA_character_,
      pub_date = x$pub_date %||% NA_character_,
      headline_main = x$headline$main %||% NA_character_,
      snippet = x$snippet %||% NA_character_,
      keywords = if (!is.null(x$keywords)) paste(map_chr(x$keywords, ~ .x$value %||% ""), collapse = " | ") else NA_character_
    )
  }, .id = "source_id")
